      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
        <button
          ...
          className={`px-3.5 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition-colors ${
            statusFilter === 'ALL' ? 'bg-emerald-600 text-white dark:bg-emerald-500 dark:text-slate-950 font-bold' : 'bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-400 border border-slate-200 dark:border-slate-800'
          }`}
        >
        ...
        {statuses.map((st) => {
          ...
          return (
            <button
              ...
              className={`px-3.5 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition-colors ${
                statusFilter === st ? 'bg-emerald-600 text-white dark:bg-emerald-500 dark:text-slate-950 font-bold' : 'bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-400 border border-slate-200 dark:border-slate-800'
              }`}
            >
              {st} ({count})
            </button>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/80">
              {filteredOrders.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-8 text-center text-slate-500 dark:text-slate-500">
                ...
                filteredOrders.map((ord) => (
                  <tr key={ord.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="py-3.5 px-4">
                      <span className="font-mono font-bold text-emerald-600 dark:text-emerald-400 text-sm block">{ord.order_number}</span>
                      <span className="text-[10px] text-slate-500 dark:text-slate-400">{new Date(ord.created_at).toLocaleString()}</span>
                    </td>
                    <td className="py-3.5 px-4">
                      <p className="font-bold text-slate-900 dark:text-white">{ord.customer_name}</p>
                      <p className="text-[10px] text-slate-500 dark:text-slate-400 flex items-center gap-1 mt-0.5">
                        <Phone className="w-3 h-3 text-emerald-600 dark:text-emerald-400" /> {ord.customer_phone}
                      </p>
                    </td>
                    <td className="py-3.5 px-4">
                      <div className="max-w-xs space-y-0.5">
                        {ord.items.map((it) => (
                          <p key={it.id} className="text-slate-700 dark:text-slate-300 text-[11px] truncate">
                            {it.product_name} <strong className="text-slate-900 dark:text-white">x{it.quantity}</strong>
                          </p>
                        ))}
                      </div>
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <select
                        ...
                        className="bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 font-bold text-xs rounded-xl px-2.5 py-1.5 focus:outline-hidden"
                      >
                        {statuses.map((st) => (
