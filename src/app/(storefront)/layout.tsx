  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-950 text-slate-100 font-sans">
      ...
      <header className="sticky top-0 z-40 bg-slate-900/95 backdrop-blur-md border-b border-slate-800 shadow-lg w-full">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 xl:px-8 h-16 sm:h-20 flex items-center justify-between gap-2 md:gap-3 lg:gap-4 w-full box-border min-w-0">
          {/* Logo & Brand */}
          <Link href="/" className="flex items-center gap-2 sm:gap-3 group shrink min-w-0">
            ...
            <div className="min-w-0">
              <span className="font-extrabold text-sm sm:text-base lg:text-lg tracking-tight bg-gradient-to-r from-white via-slate-100 to-emerald-400 bg-clip-text text-transparent truncate block">
                {shop.name}
              </span>
              <span className="hidden sm:block text-[10px] sm:text-xs text-emerald-400 font-medium truncate">
                Local Retail Shop
              </span>
            </div>
          </Link>
          ...
          {/* Desktop Search Bar */}
          <form onSubmit={handleSearchSubmit} className="hidden md:flex flex-1 min-w-0 max-w-[200px] lg:max-w-[280px] xl:max-w-md relative mx-1 lg:mx-2">
            <input
              type="text"
              placeholder="Search rice, oils, milk..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-slate-800/90 text-slate-100 placeholder-slate-400 text-xs sm:text-sm rounded-full pl-9 pr-3 py-2 border border-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all shadow-inner truncate"
            />
            <Search className="w-4 h-4 text-slate-400 absolute left-3 top-2.5 pointer-events-none" />
          </form>

          {/* Nav Links */}
          <nav className="hidden lg:flex items-center gap-2 xl:gap-4 shrink-0 min-w-0">
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-1.5 text-xs xl:text-sm font-semibold transition-colors px-2 py-1 rounded-lg ${
                    isActive ? 'text-emerald-400 bg-slate-800/50' : 'text-slate-300 hover:text-white hover:bg-slate-800/30'
                  }`}
                >
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 lg:gap-3 shrink-0 min-w-0">
            ...
            <Link
              href="/cart"
              className="relative bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-400 border border-emerald-500/30 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl flex items-center gap-1.5 font-semibold text-xs sm:text-sm transition-all hover:scale-105 shrink-0"
            >
            ...
            {currentCustomer ? (
              <Link
                href="/account"
                className="hidden sm:flex items-center gap-1.5 bg-slate-800 hover:bg-slate-700 text-slate-100 border border-slate-700 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl text-xs font-bold transition-colors shrink-0 max-w-[110px] sm:max-w-[140px]"
                title={currentCustomer.name}
              >
                <User className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-400 shrink-0" />
                <span className="max-w-[60px] sm:max-w-[80px] xl:max-w-[100px] truncate">{currentCustomer.name.split(' ')[0]}</span>
              </Link>
            ) : (
              <Link
                href="/login"
                className="hidden sm:flex items-center gap-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl text-xs font-semibold transition-colors shrink-0"
              >
                <User className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-400 shrink-0" />
            ...
            <Link
              ...
              className="hidden sm:flex items-center gap-1.5 bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-500 hover:to-teal-600 text-slate-950 font-black px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-xl text-xs transition-transform hover:scale-105 shadow shrink-0"
            >
