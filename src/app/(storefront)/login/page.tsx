    ...
    <div className="max-w-md mx-auto px-4 py-12 space-y-8 min-h-[70vh] flex flex-col justify-center">
      ...
      <div className="text-center space-y-2">
        <Link href="/" className="inline-flex items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-700 flex items-center justify-center text-white font-black shadow-lg shadow-emerald-500/20">
            <User className="w-6 h-6" />
          </div>
          <span className="font-extrabold text-2xl tracking-tight text-slate-900 dark:text-white">{shop.name}</span>
        </Link>
        <h1 className="text-xl font-extrabold text-slate-900 dark:text-white">Customer Portal Authentication</h1>
        <p className="text-xs text-slate-500 dark:text-slate-400">Enter your credentials to log in or register a new customer account.</p>
      </div>

      {/* ERROR NOTICE DISPLAY */}
      {errorNotice && (
        <div className="bg-red-50 dark:bg-red-950/80 border border-red-200 dark:border-red-500/50 text-red-700 dark:text-red-300 p-4 rounded-2xl text-xs font-bold space-y-2 shadow-xs dark:shadow-xl">
          <div className="flex items-start gap-2.5">
            ...
            <div className="space-y-1">
              ...
              <p className="text-red-700 dark:text-red-300 leading-relaxed">{errorNotice}</p>
            </div>
          </div>
          ...
          {errorType === 'NOT_REGISTERED' && (
            <button
              ...
              className="w-full bg-emerald-600 hover:bg-emerald-500 text-white dark:text-slate-950 font-black py-2 rounded-xl text-xs flex items-center justify-center gap-1.5 transition-colors shadow-xs"
            >
      ...
      {successNotice && (
        <div className="bg-emerald-100 dark:bg-emerald-500/20 border border-emerald-300 dark:border-emerald-500/40 text-emerald-800 dark:text-emerald-400 p-4 rounded-2xl text-xs font-bold text-center flex items-center justify-center gap-2 shadow-xs">
          <CheckCircle2 className="w-5 h-5 shrink-0" /> {successNotice}
        </div>
      )}

      {/* CUSTOMER LOGIN & REGISTRATION CARD */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xs dark:shadow-2xl">
        ...
        {customerMode === 'login' ? (
          <form onSubmit={handleCustomerLogin} className="space-y-4 text-xs" autoComplete="off">
            <div>
              <label className="font-semibold text-slate-700 dark:text-slate-300 block mb-1">Registered Mobile Number or Email *</label>
              <div className="relative">
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl pl-10 pr-4 py-2.5 focus:outline-hidden focus:ring-2 focus:ring-emerald-500 font-medium"
                />
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-500 hover:to-teal-600 text-white dark:text-slate-950 font-black py-3.5 rounded-xl shadow-lg shadow-emerald-500/20 text-xs flex items-center justify-center gap-2 transition-all hover:scale-102 disabled:opacity-50"
            >
          ...
          <form onSubmit={handleCustomerRegister} className="space-y-4 text-xs">
            <div className="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 p-3 rounded-2xl text-[11px] text-emerald-800 dark:text-emerald-300 font-semibold flex items-center gap-2">
            ...
            <div>
              <label className="font-semibold text-slate-700 dark:text-slate-300 block mb-1">Mobile Phone Number *</label>
              <div className="relative">
                <input
                  type="tel"
                  required
            ...
            <div>
              <label className="font-semibold text-slate-700 dark:text-slate-300 block mb-1">Default Delivery Address *</label>
              <div className="relative">
                <input
                  type="text"
                  required
            ...
            <div>
              <label className="font-semibold text-slate-700 dark:text-slate-300 block mb-1">Create Password *</label>
              <div className="relative">
                <input
                  type="password"
                  required
      ...
      <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl text-center space-y-1 shadow-xs">
        <p className="text-xs text-slate-500 dark:text-slate-400">Are you the Shop Owner?</p>
        <Link
          href="/owner/login"
