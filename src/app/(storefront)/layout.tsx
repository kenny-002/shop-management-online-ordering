...
export default function StorefrontLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100 font-sans transition-colors duration-200">
      ...
      {/* Main Navbar */}
      <header className="sticky top-0 z-40 bg-white/90 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 shadow-lg w-full transition-colors duration-200">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between gap-2.5 lg:gap-4 xl:gap-5 w-full box-border min-w-0">
          ...
          <Link href="/" className="flex items-center gap-2 sm:gap-2.5 group shrink-0">
            ...
            <div className="shrink-0">
              <span className="font-extrabold text-sm sm:text-base lg:text-lg tracking-tight text-slate-900 dark:bg-gradient-to-r dark:from-white dark:via-slate-100 dark:to-emerald-400 dark:bg-clip-text dark:text-transparent block">
                {shop.name}
              </span>
              <span className="hidden sm:block text-[10px] sm:text-xs text-emerald-600 dark:text-emerald-400 font-semibold">
                Local Retail Shop
              </span>
            </div>
          </Link>
          ...
          <form onSubmit={handleSearchSubmit} className="hidden md:flex relative flex-1 min-w-[150px] max-w-[190px] lg:max-w-[220px] xl:max-w-[240px]">
            <input
              type="text"
              placeholder="Search rice, oils, milk..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full h-10 bg-slate-100 dark:bg-slate-800/90 text-slate-900 dark:text-slate-100 placeholder-slate-500 dark:placeholder-slate-400 text-xs sm:text-sm rounded-full pl-9 pr-3.5 border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all shadow-inner truncate"
            />
            <Search className="w-4 h-4 text-slate-400 dark:text-slate-400 absolute left-3 top-3 pointer-events-none" />
          </form>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`flex items-center gap-1.5 text-xs xl:text-sm font-semibold transition-colors px-2.5 py-1.5 rounded-lg ${
                    isActive ? 'text-emerald-700 dark:text-emerald-400 font-bold bg-slate-100 dark:bg-slate-800/60' : 'text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800/40'
                  }`}
                >
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5 lg:gap-3 shrink-0">
            ...
            <Link
              href="/cart"
              className="relative bg-emerald-100 dark:bg-emerald-600/20 hover:bg-emerald-200 dark:hover:bg-emerald-600/30 text-emerald-800 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-500/30 px-2.5 sm:px-3 py-2 rounded-xl flex items-center gap-1.5 font-semibold text-xs sm:text-sm transition-all hover:scale-105 shrink-0"
            >
            ...
            {currentCustomer ? (
              <Link
                href="/account"
                className="hidden sm:flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-100 border border-slate-300 dark:border-slate-700 px-2.5 sm:px-3 py-2 rounded-xl text-xs font-bold transition-colors shrink-0"
              ...
              <Link
                href="/login"
                className="hidden sm:flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 border border-slate-300 dark:border-slate-700 px-2.5 sm:px-3 py-2 rounded-xl text-xs font-semibold transition-colors shrink-0"
              >
                <User className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-600 dark:text-emerald-400 shrink-0" />
            ...
            {/* OWNER PORTAL / DASHBOARD BUTTON */}
            <Link
              href={isOwnerLoggedIn ? '/owner/dashboard' : '/owner/login'}
              className="hidden sm:flex items-center gap-1.5 bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-500 hover:to-teal-600 text-white dark:text-slate-950 font-black px-3 sm:px-3.5 py-2 rounded-xl text-xs transition-transform hover:scale-105 shadow shrink-0"
            >
            ...
            <button
              ...
              className="xl:hidden p-2 text-slate-700 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-xl transition-colors shrink-0"
        ...
        {mobileMenuOpen && (
          <div className="xl:hidden bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 pt-3 pb-6 space-y-4 shadow-xl">
            <form onSubmit={handleSearchSubmit} className="relative">
              <input
                ...
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-500 dark:placeholder-slate-400 text-sm rounded-xl pl-10 pr-4 py-2.5 border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500"
              />
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-2.5 p-3 rounded-xl text-sm font-medium transition-colors ${
                      isActive ? 'bg-emerald-500/10 dark:bg-emerald-600/20 text-emerald-700 dark:text-emerald-400 border border-emerald-500/30' : 'bg-slate-100 dark:bg-slate-800/60 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-800 dark:text-slate-200'
                    }`}
                  >
            ...
            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-200 dark:border-slate-800">
              <Link
                ...
                onClick={() => setMobileMenuOpen(false)}
                className="flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-300 dark:border-slate-700 py-2.5 px-3 rounded-xl text-xs font-semibold transition-colors truncate"
              >
              ...
              <Link
                href={isOwnerLoggedIn ? '/owner/dashboard' : '/owner/login'}
                onClick={() => setMobileMenuOpen(false)}
                className="flex items-center justify-center gap-2 bg-gradient-to-r from-emerald-600 to-teal-700 text-white dark:text-slate-950 font-bold py-2.5 px-3 rounded-xl text-xs hover:from-emerald-500 hover:to-teal-600 transition-all truncate shadow"
              >
