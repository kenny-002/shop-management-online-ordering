  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-3xl shadow-xl transition-colors">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white">Settings → Bill & Store Options</h1>
          <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">Configure shop Google Maps location, automatic digital bill delivery, and messaging providers.</p>
        </div>
      ...
      <form onSubmit={handleSaveSettings} className="space-y-8">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 space-y-4 shadow-xl transition-colors">
          <h3 className="font-extrabold text-base text-slate-900 dark:text-white border-b border-slate-200 dark:border-slate-800 pb-3 flex items-center gap-2">
            <Bell className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> Bill & Mobile Notification Options
          </h3>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-950 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-800 transition-colors">
              <input
                type="checkbox"
                id="autoGen"
              ...
              />
              <label htmlFor="autoGen" className="font-bold text-slate-900 dark:text-white cursor-pointer">
            ...
            <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-950 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-800 transition-colors">
              <input
                ...
                className="w-4 h-4 accent-emerald-500 rounded"
              />
              <label htmlFor="autoSend" className="font-bold text-slate-900 dark:text-white cursor-pointer">
                ☑ Automatically send bill to customer mobile number
              </label>
            </div>

            <div>
              <label className="font-semibold text-slate-700 dark:text-slate-300 block mb-1">Preferred Delivery Method</label>
              <select
                ...
                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl px-4 py-2.5 font-bold text-emerald-600 dark:text-emerald-400 outline-none focus:border-amber-500 transition-colors"
              >
                <option value="WHATSAPP" className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">○ WhatsApp Business API</option>
                <option value="SMS" className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">○ SMS Provider</option>
                <option value="BOTH" className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">○ Both (WhatsApp with SMS Fallback)</option>
              </select>
            </div>

            <div>
              <label className="font-semibold text-slate-700 dark:text-slate-300 block mb-1">Invoice Number Prefix</label>
              <input
                type="text"
                value={invoicePrefix}
                onChange={(e) => setInvoicePrefix(e.target.value)}
                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl px-4 py-2.5 font-mono font-bold outline-none focus:border-amber-500 transition-colors"
              />
            </div>

            <div className="sm:col-span-2">
              <label className="font-semibold text-slate-700 dark:text-slate-300 block mb-1">Customer SMS / WhatsApp Message Template</label>
              <textarea
                rows={3}
                value={messageTemplate}
                onChange={(e) => setMessageTemplate(e.target.value)}
                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-xl px-4 py-2.5 text-xs font-mono outline-none focus:border-amber-500 transition-colors"
              />
              <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">Available tags: [SHOP NAME], [ORDER_NO], [AMOUNT], [INVOICE_LINK]</p>
            </div>

            <div>
              <label className="font-semibold text-slate-700 dark:text-slate-300 block mb-1 flex items-center gap-1">
                <Key className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> WhatsApp Business API Token
              </label>
              <input
                type="password"
                ...
                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl px-4 py-2.5 text-xs font-mono outline-none focus:border-amber-500 transition-colors"
              />
            </div>

            <div>
              <label className="font-semibold text-slate-700 dark:text-slate-300 block mb-1 flex items-center gap-1">
                <Key className="w-3.5 h-3.5 text-teal-600 dark:text-teal-400" /> SMS Provider API Key (Fast2SMS / Twilio)
              </label>
              <input
                type="password"
                ...
                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl px-4 py-2.5 text-xs font-mono mb-2 outline-none focus:border-amber-500 transition-colors"
              />

              {/* Real-time SMS Test Tool */}
              <div className="bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-xl p-3 space-y-2 mt-2 transition-colors">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-bold text-slate-600 dark:text-slate-400 flex items-center gap-1">
                    <Smartphone className="w-3 h-3 text-teal-600 dark:text-teal-400" /> Test Live Fast2SMS Dispatch
                  </span>
                  {testSmsResult && (
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                        testSmsResult.success
                          ...
                          : 'bg-rose-500/10 dark:bg-rose-500/20 text-rose-700 dark:text-rose-400 border border-rose-500/30'
                      }`}
                    >
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-xs rounded-lg px-3 py-1.5 font-mono outline-none focus:border-amber-500 transition-colors"
                  />
                  <button
                    type="button"
                  ...
                  <p
                    className={`text-[11px] font-mono p-2 rounded-lg ${
                      testSmsResult.success
                        ...
                        : 'bg-rose-50 dark:bg-rose-950/50 text-rose-800 dark:text-rose-300 border border-rose-200 dark:border-rose-800/50'
                    }`}
                  >
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 space-y-4 shadow-xl transition-colors">
          <h3 className="font-extrabold text-base text-slate-900 dark:text-white border-b border-slate-200 dark:border-slate-800 pb-3 flex items-center gap-2">
            <Store className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> Store Profile & Google Maps Location
          </h3>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div className="sm:col-span-2">
              <label className="font-semibold text-slate-700 dark:text-slate-300 block mb-1">Shop Name *</label>
              <input
                type="text"
                required
                value={name}
                onChange={(e) => setName(e.target.value)}
                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl px-4 py-2.5 font-bold outline-none focus:border-amber-500 transition-colors"
              />
            </div>

            <div className="sm:col-span-2">
              <label className="font-semibold text-slate-700 dark:text-slate-300 block mb-1 flex items-center gap-1">
                <MapPin className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> Official Google Maps Location URL *
              </label>
              <div className="flex gap-2">
                <input
                  type="url"
                  ...
                  className="flex-1 bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-emerald-700 dark:text-emerald-400 font-mono text-xs rounded-xl px-4 py-2.5 font-bold outline-none focus:border-amber-500 transition-colors"
                />
                <a
                  href={googleMapsUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-emerald-700 dark:text-emerald-400 border border-slate-300 dark:border-slate-700 px-3.5 py-2.5 rounded-xl font-bold flex items-center gap-1 shrink-0 transition-colors"
                >
                  Test Link <ExternalLink className="w-3.5 h-3.5" />
                </a>
              </div>
              <span className="text-[10px] text-slate-500 dark:text-slate-400 mt-1 block">Live link used across Get Directions buttons and Store Location page.</span>
            </div>

            <div className="sm:col-span-2">
              <label className="font-semibold text-slate-700 dark:text-slate-300 block mb-1">Short Shop Description</label>
              <textarea
                rows={2}
                value={description}
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 space-y-4 shadow-xl transition-colors">
          <h3 className="font-extrabold text-base text-slate-900 dark:text-white border-b border-slate-200 dark:border-slate-800 pb-3 flex items-center gap-2">
            <QrCode className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> Shop UPI ID & QR Code Settings
          </h3>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-xs items-center">
            <div className="space-y-4">
              <div>
                <label className="font-semibold text-slate-700 dark:text-slate-300 block mb-1">Shop Official UPI ID *</label>
                <input
                  type="text"
                  required
                  value={upiId}
                  onChange={(e) => setUpiId(e.target.value)}
                  className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-emerald-700 dark:text-emerald-400 rounded-xl px-4 py-2.5 font-mono text-sm font-bold outline-none focus:border-amber-500 transition-colors"
                />
              </div>

              <div>
                <label className="font-semibold text-slate-700 dark:text-slate-300 block mb-1">QR Code Image URL *</label>
                <input
                  type="url"
                  required
                  value={qrCodeUrl}
            ...
            <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 text-center space-y-2 transition-colors">
              <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold block">Live QR Preview</span>
              <div className="bg-white p-2 rounded-xl w-36 h-36 mx-auto border border-slate-200 dark:border-slate-700 flex items-center justify-center">
                <img src={qrCodeUrl} alt="QR Code Preview" className="w-full h-full object-contain" />
              </div>
              <span className="text-xs text-emerald-700 dark:text-emerald-400 font-bold font-mono block">{upiId}</span>
            </div>
          </div>
        </div>
